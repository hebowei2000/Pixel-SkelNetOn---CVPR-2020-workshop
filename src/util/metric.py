...
class AvgMeter(object):
    ...
    def show(self):
        # print(len(self.losses) - self.num)
        # print(np.maximum(len(self.losses)-self.num, 0))
        # print(self.losses[np.maximum(len(self.losses)-self.num, 0):])
        tmp = self.losses[np.maximum(len(self.losses) - self.num, 0):]
        sum = 0.0
        for i in tmp: sum += i.item()
...
class cal_maxF(object):
    # max Fmeasure
    def __init__(self, num, thds=255, style=True):
        self.num = num
        self.style = style
        self.thds = thds
    ...
    def show(self):
        ...
        if self.style:
            ...
            fmeasure = 1.3 * self.precision * self.recall / (0.3 * self.precision + self.recall + 1e-8)
            return fmeasure.mean(axis=0).max()
